...
ClapTrap::ClapTrap() : 
	...
	_energyPoint(10),
	_attackDamage(){
		std::cout << this->_name << " CT object created" << std::endl;
}
...
ClapTrap::ClapTrap(const ClapTrap &other) {
	*this = other;
	std::cout << this->_name << " CL coped " << std::endl;
}

ClapTrap::~ClapTrap(){
	std::cout << this->_name << " CT object destroyed" << std::endl;
}
...
void ClapTrap::attack(const std::string &target) {
	if (this->_hitPoint == 0) {
		std::cout << this->_name << " CT: dead head can't attack!" << std::endl;
		return ;
	}
	else {
...
void ClapTrap::takeDamage(unsigned int amount){
	if (this->_hitPoint == 0) {
		std::cout << this->_name << " CT: dead head can't get damaged!" << std::endl;
		return ;
	}
	if (amount > _hitPoint)
		amount = _hitPoint;
